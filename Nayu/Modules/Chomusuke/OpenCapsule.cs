        ...
        public async Task OpenChomusukeCapsule(string arg)
        {
            ...
            if (!config.Chomusuke3.Have) //if they own a Chomusuke or not
            {
                ...
                if (arg == "normal")
                {
                    ...
                    else
                    {
                        await SendMessage(Context, null,
                            $":octagonal_sign:  **|**  **{Context.User.Username}**, you don't have any Normal Chomusuke Capsules!");
                        return;
                    }
                }
                ...
                if (arg == "shiny")
                {
                    ...
                    {
                        await SendMessage(Context, null,
                            $":octagonal_sign:  **|**  **{Context.User.Username}**, you don't have any Shiny Chomusuke Capsules!");
                        return;
                    }
                }
                ...
                if (arg == "mythical")
                {
                    ...
                    {
                        await SendMessage(Context, null,
                            $":octagonal_sign:  **|**  **{Context.User.Username}**, you don't have any Mythical Chomusuke Capsules!");
                        return;
                    }
                }

                var trait = GetTraitsTypes.GetTraitsAsync();
                var type = GetTraitsTypes.GetTypeAsync(value);
                var randomIndex = Global.Rng.Next(Zodiac.Length);
                var zodiac = Zodiac[randomIndex];
                var cp = 20 * (config.LevelNumber / 10); //cp is boosted based off their user level *.1
                ...
                if (responsee.Content.Equals("yes", StringComparison.CurrentCultureIgnoreCase) &&
                    (responsee.Author.Equals(Context.User)))
                {
                    ...
                    if (responseee.Author.Equals(Context.User))
                    {
                        ...
                        GlobalUserAccounts.SaveAccounts(config.Id);
                        await SendMessage(Context, null,
                            $"{Global.EChomusuke}  |  **{Context.User.Username}**, successfully named your new Chomusuke to **{responseee}**! You can use the command `n!cHelp` for more Chomusuke commands!");
                    }
                }

                if (responsee.Content.Equals("no", StringComparison.CurrentCultureIgnoreCase) &&
                    (responsee.Author.Equals(Context.User)))
                {
                    await SendMessage(Context, null,
                        $"{Global.EChomusuke}  |  **{Context.User.Username}**, your Chomusuke is, well, Chomusuke! You can change the name of it by using the command `n!cName`");
                    return;
                }
            }
