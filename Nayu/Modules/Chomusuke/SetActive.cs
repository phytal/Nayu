...
namespace Nayu.Modules.Chomusuke
{
    public class SetActive : NayuModule
    {
        ...
        public async Task ReplaceBlessing()
        {
            ...
            {
                config.ActiveChomusuke = config.Chomusuke2;
            }
            if (response.Content.Equals("3", StringComparison.CurrentCultureIgnoreCase) && (response.Author.Equals(Context.User)))
            {
                config.ActiveChomusuke = config.Chomusuke3;
            }
            else
            {
                await Context.Channel.SendMessageAsync($"You don't have that Chomusuke! \n*Make sure you typed the number correctly!*");
                return;
            }
            GlobalUserAccounts.SaveAccounts();
            await Context.Channel.SendMessageAsync($":white_check_mark:  | Successfully made Chomusuke #{config.ActiveChomusuke} your active Chomusuke.");
        }
    }
}
